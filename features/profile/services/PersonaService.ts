...
export class PersonaService {
  static async detectPersona(
    ...
    const suggestedActivities = this.getSuggestedActivities(personaType, locale);
  ...
  private static getSuggestedActivities(
    ...
    const baseActivities = {
      early_bird: [
        'Morning Workout', 'Meditation', 'Reading', 'Planning Day',
        'Healthy Breakfast', 'Journaling', 'Stretching'
      ],
      night_owl: [
        'Creative Work', 'Learning', 'Social Time', 'Entertainment',
        'Reflection', 'Planning Tomorrow', 'Wind Down'
      ],
      balanced: [
        'Exercise', 'Work Focus', 'Social Connection', 'Self Care',
        'Learning', 'Hobbies', 'Family Time'
      ],
      flexible: [
        'Mindfulness', 'Physical Activity', 'Creative Expression',
        'Social Interaction', 'Skill Building', 'Rest & Recovery'
      ]
    };
  ...
  static validateTimeRange(wakeTime: string, sleepTime: string): {
    isValid: boolean;
    error?: string;
  } {
    const wakeHour = parseInt(wakeTime.split(':')[0]);
