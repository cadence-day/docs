-- Create update function for the "internal" schema and add triggers to all the tables. Update functions are used to set the `updated_at` column to the current timestamp on insert and update version_no.
...
-- Create triggers for all tables in the "internal" schema
-- This trigger will automatically update the updated_at column and increment version_no on UPDATE operations

-- Activities table trigger
DROP TRIGGER IF EXISTS trigger_update_activities ON "internal"."activities";
CREATE TRIGGER trigger_update_activities
    BEFORE UPDATE ON "internal"."activities"
    FOR EACH ROW
    EXECUTE FUNCTION internal.update_row();

-- Activity Categories table trigger  
DROP TRIGGER IF EXISTS trigger_update_activity_categories ON "internal"."activity_categories";
CREATE TRIGGER trigger_update_activity_categories
    BEFORE UPDATE ON "internal"."activity_categories"
    FOR EACH ROW
    EXECUTE FUNCTION internal.update_row();

-- Notes table trigger
DROP TRIGGER IF EXISTS trigger_update_notes ON "internal"."notes";
CREATE TRIGGER trigger_update_notes
    BEFORE UPDATE ON "internal"."notes"
    FOR EACH ROW
    EXECUTE FUNCTION internal.update_row();

-- Profiles table trigger
DROP TRIGGER IF EXISTS trigger_update_profiles ON "internal"."profiles";
CREATE TRIGGER trigger_update_profiles
    BEFORE UPDATE ON "internal"."profiles"
    FOR EACH ROW
    EXECUTE FUNCTION internal.update_row();

-- States table trigger
DROP TRIGGER IF EXISTS trigger_update_states ON "internal"."states";
CREATE TRIGGER trigger_update_states
    BEFORE UPDATE ON "internal"."states"
    FOR EACH ROW
    EXECUTE FUNCTION internal.update_row();

-- Timeslices table trigger
DROP TRIGGER IF EXISTS trigger_update_timeslices ON "internal"."timeslices";
CREATE TRIGGER trigger_update_timeslices
    BEFORE UPDATE ON "internal"."timeslices"
    FOR EACH ROW
    EXECUTE FUNCTION internal.update_row();
...
CREATE TRIGGER trigger_update_user_preferences
    BEFORE UPDATE ON "internal"."user_preferences"
    FOR EACH ROW
    EXECUTE FUNCTION internal.update_row();


-- CREATE a function on INSERT on Auth.user to create a profile in the internal.profiles table
...
-- Create a trigger on Auth.user to call the function on INSERT
DROP TRIGGER IF EXISTS trigger_create_profile ON auth.users;
CREATE TRIGGER trigger_create_profile
    AFTER INSERT ON auth.users
    FOR EACH ROW
    EXECUTE FUNCTION internal.create_profile_on_user_insert();
-- CREATE a function on UPDATE on Auth.user to update the profile in the internal.profiles table
...
END;
$$;
-- Create a trigger on Auth.user to call the function on UPDATE
DROP TRIGGER IF EXISTS trigger_update_profile ON auth.users;
CREATE TRIGGER trigger_update_profile
    AFTER UPDATE ON auth.users
    FOR EACH ROW
    EXECUTE FUNCTION internal.update_profile_on_user_update();

-- Create a function that create a XML file with all the data from the user. 
...
CREATE TRIGGER trigger_link_timeslice_note
    AFTER INSERT ON internal.notes
    FOR EACH ROW
    EXECUTE FUNCTION internal.link_timeslice_note();
...
CREATE TRIGGER trigger_link_timeslice_state
    AFTER INSERT ON internal.states
    FOR EACH ROW
    EXECUTE FUNCTION internal.link_timeslice_state();

-- Create a function to set up a user's initial preferences
...
-- Create a trigger to set up user preferences after a new user is created
