...
using (((bucket_id = 'profile-pictures'::text) AND ((auth.uid())::text = (storage.foldername(name))[1])));
  ...
  to public
using (((bucket_id = 'profile-pictures'::text) AND ((auth.uid())::text = (storage.foldername(name))[1])))
with check (((bucket_id = 'profile-pictures'::text) AND ((auth.uid())::text = (storage.foldername(name))[1]) AND ((storage.extension(name) = 'jpg'::text) OR (storage.extension(name) = 'jpeg'::text) OR (storage.extension(name) = 'png'::text) OR (storage.extension(name) = 'webp'::text))));
