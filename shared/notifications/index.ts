...
export { useNotificationScheduler, useAutoNotificationScheduler } from './hooks/useNotificationScheduler';
...
export const createScheduledDate = (hour: number, minute: number, daysFromNow = 0): Date => {
  const date = new Date();
...
export const createWeeklyScheduledDate = (weekday: number, hour: number, minute: number): Date => {
  const date = new Date();
